...
void problem10(){
  auto f = [](double x){ return std::exp(x)*std::pow(x-1,2); };
  auto df = [](double x){ return 2*(x-1)*std::exp(x) + std::exp(x)*std::pow(x-1,2);};
  double root = mathx::roots::newtons_method(f, df, 0.0, std::pow(10,-8), 50);
  std::cout << "root of f(x) = " << root << std::endl;
  std::cout << "e_abs = " << mathx::utils::error::e_abs(1., root) << std::endl;
}
